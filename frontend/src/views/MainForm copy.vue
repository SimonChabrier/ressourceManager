...
// si je n'ai pas d'id dans la route, je crée un nouveau post

if (!router.currentRoute.value.params.id) {
    const button = document.querySelector('input[type="submit"]');
    button.addEventListener('click', createRessource);
} 
else {
    const button = document.querySelector('input[type="submit"]');
    button.addEventListener('click', patchRessource);
}
...
const patchRessource = async () => {
  ...
  await ressourcesStore.patchRessource(ressourceId, ressource);
  router.push({ name: 'ressources' });
};
